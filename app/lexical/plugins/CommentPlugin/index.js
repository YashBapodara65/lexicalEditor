    ...
    editor.getEditorState().read(() => {
      ...
      if ($isRangeSelection(selection)) {
        ...
        nodeData = nodes.map((node) => ({
          id: node.getKey ? node.getKey() : null,
          type: node.getType ? node.getType() : "unknown",
          text: node.getTextContent ? node.getTextContent() : "",
        }));

        // Build the quote from full text nodes, not just selection
        quote = nodes
          .map((node) => {
            if ($isTextNode(node)) {
              return node.getTextContent(); // full text of node
            } else {
              return node.getTextContent ? node.getTextContent() : "";
            }
          })
          .join(" "); // join multiple nodes with space if needed

        // Truncate if too long
        if (quote.length > 100) quote = quote.slice(0, 99) + "…";

        console.log("🧩 Node data:", nodes);
        console.log("💬 Full node quote:", quote);
      }
    });

    // ✅ Combine quote + input comment text
    const combinedText = quote ? `"${quote}" — ${content}` : content;

    console.log("✏️ Comment text to submit:", combinedText);

    // Submit the comment
    submitAddComment(
      createThread(combinedText, [createComment(content, author)]),
      true,
      undefined,
...
export default function CommentPlugin({ providerFactory }) {
  ...
  const markNodeMap = useMemo(() => new Map(), []);
        ...
        editor.update(() => {
          ...
          if ($isRangeSelection(selection)) {
            ...
            if ($isTextNode(anchorNode)) {
              anchorNode.select(); // selects the full text node
              const isBackward = selection.isBackward();
              const id = commentOrThread.id;
              $wrapSelectionInMarkNode($getSelection(), isBackward, id);
            }
          }
        });
  ...
  useEffect(() => {
    ...
    return () => {
      for (let i = 0; i < changedElems.length; i++) {
        changedElems[i].classList.remove("selected");
      }
    };
