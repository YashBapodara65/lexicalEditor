...
const toggleTableCellBorder = (editor, side) => {
  editor.update(() => {
    const selection = $getSelection();
    ...
    const selectedNodes =
      $isTableSelection(selection)
        ? selection.getNodes()
        : [$getNodeTriplet(selection.anchor)[0]];

    selectedNodes.forEach((cellNode) => {
      if (!cellNode || cellNode.getType() !== "custom-tablecell") return;

      const [_, rowNode, tableNode] = $getNodeTriplet(cellNode);

      const allRows = tableNode.getChildren();
      const rowIndex = allRows.indexOf(rowNode);
      const colIndex = rowNode.getChildren().indexOf(cellNode);

      const currentStyle = cellNode.getStyle() || "";

      // ✅ FIX: Proper toggle with restore
      const toggleStyle = (style, side) => {
        ...
        const currentValue = styleObj[key];
      ...
      cellNode.setStyle(toggleStyle(currentStyle, side));
      ...
      switch (side) {
        case "top":
          oppositeSide = "bottom";
          if (rowIndex > 0) adjCell = allRows[rowIndex - 1].getChildren()[colIndex];
          break;
        case "bottom":
          oppositeSide = "top";
          if (rowIndex < allRows.length - 1) adjCell = allRows[rowIndex + 1].getChildren()[colIndex];
          break;
        case "left":
          oppositeSide = "right";
          if (colIndex > 0) adjCell = rowNode.getChildren()[colIndex - 1];
          break;
        case "right":
          oppositeSide = "left";
          if (colIndex < rowNode.getChildren().length - 1) adjCell = rowNode.getChildren()[colIndex + 1];
          break;
      }

      if (adjCell) {
        const adjStyle = adjCell.getStyle() || "";
        adjCell.setStyle(toggleStyle(adjStyle, oppositeSide));
      }
    });
  });
};
  ...
  const clearTableSelection = useCallback(() => {
    editor.update(() => {
      ...
      const latestCellNode = tableCellNode.getLatest();
      if (!latestCellNode.isAttached()) return;

      const tableNode = $getTableNodeFromLexicalNodeOrThrow(latestCellNode);
      if (!tableNode.isAttached()) return;

      // Now get writable safely
      const writableTableNode = tableNode.getWritable();
      writableTableNode.markDirty();

      updateTableCellNode(latestCellNode.getLatest());
      $setSelection(null);
    });
  }, [editor, tableCellNode]);
...
const handleCellBackgroundColor = useCallback(
  ...
  const formatVerticalAlign = useCallback(
    (value) => {
      editor.update(() => {
        const selection = $getSelection();
        if ($isRangeSelection(selection) || $isTableSelection(selection)) {
  ...
  type="button"
  className="item"
  onClick={() => toggleTableCellBorder(editor, "top")}
>
...
<button
  type="button"
  className="item"
  onClick={() => toggleTableCellBorder(editor, "bottom")}
>
...
<button
  type="button"
  className="item"
  onClick={() => toggleTableCellBorder(editor, "left")}
>
...
<button
  type="button"
  className="item"
  onClick={() => toggleTableCellBorder(editor, "right")}
>
