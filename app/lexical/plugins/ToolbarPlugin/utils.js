...
const normalizeSelectionIndent = (selection) => {
  if (!$isRangeSelection(selection)) return;
  selection.getNodes().forEach((node) => {
    if ($isListItemNode(node)) {
      if (node.__indent == null || node.__indent < 0) {
        node.setIndent(0);
      }
    }
  });
...
export const formatBulletList = (editor, blockType) => {
  editor.update(() => {
    const selection = $getSelection();
    normalizeSelectionIndent(selection);
...
export const formatNumberedList = (editor, blockType) => {
  editor.update(() => {
    ...
    normalizeSelectionIndent(selection); // optional, if you support indents

    if (blockType !== 'number') {
      // Insert a numbered list
      editor.dispatchCommand(INSERT_ORDERED_LIST_COMMAND, undefined);
    } else {
      // Convert back to paragraph if already a numbered list
      formatParagraph(editor);
    }
  });
};
