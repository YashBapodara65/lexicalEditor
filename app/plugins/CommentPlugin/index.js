...
function CommentInputBox({ editor, cancelAddComment, submitAddComment }) {
  ...
  const submitComment = () => {
    ...
    const combinedText = quote
      ? `"${quote}" — ${content}` // e.g., quote first, then comment
      : content; // if nothing is selected, just use the comment

    console.log("✏️ Comment text to submit:", combinedText);

    // Submit the comment with combined text
    submitAddComment(
