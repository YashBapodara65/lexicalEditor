...
export default function DateTimeComponent({ dateTime, nodeKey }) {
  ...
  const { refs, floatingStyles, context } = useFloating({
    ...
    middleware: [offset(5), flip({ fallbackPlacements: ['top-start'] }), shift({ padding: 10 })],
  ...
  useEffect(() => {
    ...
    if (dateTimePillRef) dateTimePillRef.addEventListener('click', onClick);
    return () => {
      if (dateTimePillRef) dateTimePillRef.removeEventListener('click', onClick);
    };
  }, [refs, editor]);
              ...
              >
                ...
                <form style={{ marginBlockEnd: '1em' }}>
                  <div style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', width: '300px' }}>
                    <input type="checkbox" checked={includeTime} onChange={handleCheckboxChange} />
                    <label>
